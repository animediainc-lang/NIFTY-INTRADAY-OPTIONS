...
class PreMarketAnalyzer:
    ...
    def get_option_chain_metrics(self, expiry_date: str) -> Dict[str, Any]:
        """Calculates PCR, Max Pain, and estimated ATM IV."""
        try:
            ...
            if response.get("Status") == 200 and response.get("Success"):
                ...
                # ATM IV (Simplified: Average IV of ATM Call and Put)
                # Note: Assumes 'iv' column exists in Breeze response
                if 'iv' in df.columns:
                    # Find ATM strike
                    # In a real scenario, we'd need the LTP to find ATM
                    # Here we just take the strike with highest OI as a proxy or leave as 0
                    self.metrics["atm_iv"] = df['iv'].astype(float).mean()
    ...
    def get_india_vix(self) -> float:
        ...
        try:
            response = breeze_client.get_quotes(stock_code="INDIAVIX", exchange_code="NSE", product_type="cash")
            if response.get("Status") == 200 and response.get("Success"):
                vix = float(response["Success"][0]["last"])
                self.metrics["vix"] = vix
                return vix
            return 0.0
        except Exception as e:
    ...
    def run_full_analysis(self, expiry_date: str):
        ...
        self.get_india_vix()
        logger.info(f"Pre-Market Analysis Complete. Metrics: {self.metrics}")
        return self.metrics
