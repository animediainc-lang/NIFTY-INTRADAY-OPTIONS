...
main.py
=======
Entry point — wires broker live data with paper order execution.

Morning checklist
-----------------
1. Run:  python tools/get_session_token.py     ← for ICICIdirect
   OR:   python tools/get_upstox_token.py      ← for Upstox
...
def main():
    ...
    # ── Broker client ─────────────────────────────────────────────────────────
    from broker.broker_client import create_broker_client
    try:
        broker = create_broker_client(cfg.broker)
    except Exception as e:
        logger.critical(f"Failed to create broker client: {e}")
        sys.exit(1)
    ...
    if broker_name == "icicidirect":
        ...
        # Use generic MarketDataFeed for Upstox and Paper
        from data.data_feed import MarketDataFeed
        data_feed = MarketDataFeed(
            broker_client=broker,
            paper_trading=cfg.broker.get("paper_trading", True),
            lot_size=cfg.instrument.get("lot_size", 50),
        )
    ...
    # For Breeze, we start streaming via websocket.
    # For others, MarketDataFeed starts a polling thread.
    if broker_name == "icicidirect":
        data_feed.start_streaming(list(chain.keys()), interval=2.0)
    else:
        data_feed.start_streaming(list(chain.keys()), interval=1.0)
